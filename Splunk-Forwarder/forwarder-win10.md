...
## Applying the Same Process to Domain Controller Logs

The same forwarder setup process was later applied to the Domain Controller to ingest Active Directory–related logs. Since Splunk Enterprise was already running on the Domain Controller, the Universal Forwarder was configured to forward events to the local instance instead of a remote receiver.

In this case, the forwarder output was pointed to `localhost` on port `9997`, while the input configuration remained the same. This allowed Security, System, and Application event logs generated by the Domain Controller to be collected and indexed locally without requiring any changes to the overall architecture.

Using the same forwarder configuration for both the Windows 10 client and the Domain Controller ensures consistency across hosts and simplifies log management, while providing centralized visibility into authentication activity, system behavior, and Active Directory events for detection and analysis.
